...
async def get_pydantic_model_or_raise_exception(model: Type[Model], db_obj: Optional[Any] = None) -> Model:
    if db_obj is None:
        raise HTTPException(status_code=404, detail={"success": "false", "description": "Object not found"})
    try:
        return model.from_orm(db_obj)  # type: ignore
    except ValidationError:
        raise HTTPException(status_code=400, detail={"success": "false", "description": "Server validation error"})
