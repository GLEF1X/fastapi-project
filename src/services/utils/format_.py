import json
from typing import List, Any, Type, TypeVar, Dict, Iterable

_T = TypeVar("_T")
BoundedDict = TypeVar("BoundedDict", bound=Dict[Any, Any])


def into_complex_obj(lst_of_objects: Iterable[Any], model: Type[_T]) -> List[_T]:
    """
    ...
    """
    return [model.from_orm(obj) for obj in lst_of_objects]  # type: ignore
...
def dict_(**dictionary: BoundedDict) -> BoundedDict:
    ...
    """
    return {k: str(v) for k, v in dictionary.items() if v is not None}  # type: ignore
